...
function init() {
    ...
    resetPlayerState();
    // requestAnimationFrame(gameLoop); // Disabled for Gym
}
...
window.resetGame = function () {
    wallRenderer.clear();
    resetPlayerState();
    // Do one render to ensure everything is initialized
    render();
...
window.step = function (action) {
    ...
    // Decode Action
    // 0: Limb Selector (0-3 mapped to strings)
    // 1: Target DX
    // 2: Target DY
    // 3: Grab Trigger
    // 4: Piton Trigger
    ...
    // 1. Select Limb
    const limbIdx = Math.floor(((action[0] + 1) / 2) * 4); // Map -1..1 to 0..3.99
    const limbs = ['leftArm', 'rightArm', 'leftLeg', 'rightLeg'];
    const selectedLimb = limbs[Math.max(0, Math.min(3, limbIdx))];

    if (gameState.selectedLimb !== selectedLimb) {
        // Emulate switching logic
        const oldLimb = player.limbs[gameState.selectedLimb];
        // Reset release state of old limb if needed? 
        // The original game logic does this on key press.
        gameState.selectedLimb = selectedLimb;
        updateHUD();
    }

    // 2. Move Mouse Cursor (Target)
    // Map -1..1 to max reach radius (approx 100px around player)
    const reachRadius = 150;
    const targetX = player.x + action[1] * reachRadius;
    const targetY = player.y + action[2] * reachRadius;
    gameState.mousePos.x = targetX;
    gameState.mousePos.y = targetY;

    // 3. Grab / Release
    const grabTrigger = action[3];
    const currentLimb = player.limbs[gameState.selectedLimb];

    if (grabTrigger > 0.5) {
        // Attempt Grab
        if (!currentLimb.grabbedAt && !currentLimb.onGround) {
            const stickiness = getGrabbabilityAt(currentLimb.x, currentLimb.y);
            if (stickiness >= CONFIG.grabThreshold) {
                // Check constraints
                let valid = true;
                if (gameState.selectedLimb.includes('Leg')) {
                    if (currentLimb.y < player.y - CONFIG.torsoLength / 2) valid = false;
                }

                if (valid) {
                    currentLimb.grabbedAt = { x: currentLimb.x, y: currentLimb.y, stickiness };
                    currentLimb.previousGrab = null;
                    currentLimb.wasReleased = false;
                }
            }
        }
    } else {
        // Release
        if (currentLimb.grabbedAt || currentLimb.onGround) {
            currentLimb.grabbedAt = null;
            currentLimb.onGround = false;
            currentLimb.wasReleased = true;
            currentLimb.previousGrab = null;
        }
    }

    // 4. Piton
    if (action[4] > 0.8) { // High threshold for consumable
        if (!gameState.falling && !gameState.gameOver && gameState.pitons.length < CONFIG.maxPitons) {
            // Rate limit? The game allows spaming S. Agent will learn.
            placePiton();
        }
    }

    // 5. Ground Move
    const moveX = action[5];
    if (gameState.onGround && Math.abs(moveX) > 0.1) {
        const speed = 3 * moveX;
        player.x += speed;
        // Sync attached ground limbs (simplified from game logic)
        ['leftLeg', 'rightLeg'].forEach(l => {
            if (player.limbs[l].onGround) player.limbs[l].x += speed;
        });
    }

    // Run Physics Loop
    // We run 1 frame per step. Gym envs often run 4 frames.
    // Let's run 4 updates per step for smoother movement?
    // Or just 1. Let's do 1 for precision control.
    update();
    render(); // Update visual state (not typically needed for headless but good for debugging)

    // Calculate Reward
    // Reward = Height Gain + Survival Bonus - Energy Cost?
    const currentHeight = Math.max(0, (CONFIG.groundY - player.y) / 50);
    const heightReward = (currentHeight - gameState.maxHeight) * 10; // Significant reward for new height
...
function getObservation() {
    // Flatten numeric state
    const numeric = [];

    // Player (4)
    numeric.push(player.x - gameState.camera.x); // Relative to screen for normalization? Or absolute? 
    // Absolute X is fine if wall is infinite. 
    // Normalized to camera is better for CNNs but we are using numeric.
    // Let's use relative to camera center?
    numeric.push(player.x);
    numeric.push(player.y);
    numeric.push(player.velocityY);
    numeric.push(gameState.stamina);

    // Limbs (4 * 3 = 12)
    ['leftArm', 'rightArm', 'leftLeg', 'rightLeg'].forEach(l => {
        const limb = player.limbs[l];
        numeric.push(limb.x - player.x); // Relative to player
        numeric.push(limb.y - player.y);
        numeric.push(limb.grabbedAt ? 1.0 : (limb.onGround ? 0.5 : 0.0));
    });

    // Grid (50x50)
    // Sample wall grabbability around player
    // Resolution: 50x50 covering say 500x500 pixels (10px per cell)
    const gridSize = 50;
    const cellSize = 10; // 10px blocks
    const grid = []; // Will be flat array, reshaped in Python

    const startX = player.x - (gridSize * cellSize) / 2;
    const startY = player.y - (gridSize * cellSize) / 2;

    for (let i = 0; i < gridSize; i++) {
        const row = [];
        for (let j = 0; j < gridSize; j++) {
            const wx = startX + j * cellSize;
            const wy = startY + i * cellSize;
            const val = getGrabbabilityAt(wx, wy);
            row.push(val);
            // Also could flatten here directly
            grid.push(val);
        }
    }
    ...
    return {
        numeric: numeric,
        grid: grid // Passed as 1D list, reshaped in Python
    };
}
